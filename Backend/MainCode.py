...
def eng2aud(text,dialougue_len,gender,j):
    ...
    engine.save_to_file(text, "Automated Video Translater/Backend/final_dialogues/dialogue_segment_"+str(i+1)+".wav")
...
if __name__ == '__main__':
    ...
    inputvideo="Automated Video Translater/Frontend/uploads/input.mp4"
    print("Loaded Video :",inputvideo)
    ffmpeg.input(inputvideo).output("Automated Video Translater/Backend/out.mp3").run(overwrite_output=True)
    print("Audio Extracted.")

#spleeter
    subprocess.run(["python", "Automated Video Translater/Backend/Spleet.py"])

#Trim

    audio_file = "Automated Video Translater/Backend/output/out/vocals.wav"
    output_dir = "Automated Video Translater/Backend/output_dialogues"
    silence_thresh = -40
...
#conversion mal audio to english audio
    ...
    MODEL_DIR = 'Automated Video Translater/Backend/GenderResnet2'
    model, label_encoder = load_model_and_encoder(MODEL_DIR)
    output_dir1 = "Automated Video Translater/Backend/final_dialogues"
    os.makedirs(output_dir1, exist_ok=True)
    dialogue_data=[] #[len,nor0/male1/female2]
    for i in range(len(dialogue_segments)):
        source_path="Automated Video Translater/Backend/output_dialogues/dialogue_segment_"+str(i+1)+".wav"
        ...
        if dial_len < 0.6:
            ...
            continue
        audio_file_path = "Automated Video Translater/Backend/output_dialogues/dialogue_segment_"+str(i+1)+".wav"
...
#merging accompaniments and audio
    ...
    accompaniment = AudioSegment.from_file(r"Automated Video Translater/Backend/output/out/accompaniment.wav")

    for i in range(len(dialogue_segments)):
        vocal = AudioSegment.from_file(rf"Automated Video Translater/Backend/final_dialogues/dialogue_segment_{i+1}.wav")
        timestamp = dialogue_segments[i][0] * 1000
        accompaniment = accompaniment.overlay(vocal, position=timestamp)
    accompaniment.export("Automated Video Translater/Backend/combined_output.wav", format="wav")
    print("Vocals and accompaniment combined")

#audio video merging
    
    video = VideoFileClip(inputvideo)
    new_audio = AudioFileClip("Automated Video Translater/Backend/combined_output.wav")
    video = video.set_audio(new_audio)
    video.write_videofile("Automated Video Translater/Frontend/output/output.mp4", codec="libx264")
